(define (script_fu_batch_brushes inDir inLoadType )
	(let*
		...
		(while (not (null? varFileList))
			...
			(set! basename (car (reverse (strbreakup (car (strbreakup filename ".")) "\\"))))

			(gimp-message basename)

			;(set! gimp-image-get-active-layer)
			;(gimp-image-flatten image)
			(set! imagetype (gimp-image-base-type image))
			(if (not (= RGB (car (gimp-image-base-type image))))
             (gimp-image-convert-rgb image))
			(set! drawable (car (gimp-image-get-active-layer image)))
			(file-gbr-save 1 image drawable newfilename "" 75 basename)
